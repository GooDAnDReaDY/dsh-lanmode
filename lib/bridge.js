...
import { REFUSED, PIN_REQUIRED, isPrivileged, verifyLanPin } from './privileged.js'
...
export function startDirectBridge(ctx, options) {
  ...
  const upgrade = (req, socket, head) => {
    ...
    upstream.on('upgrade', (answer, upstreamSocket, upstreamHead) => {
      ...
      const dropDownstream = () => { try { upstreamSocket.destroy() } catch (_) { /* уже мертво */ } }
      const dropUpstream = () => { try { socket.destroy() } catch (_) { /* уже мертво */ } }
