...
export function startDirectBridge(context, options = {}) {
  ...
  if (browserAuthSecret && !cachedCoreAuthCookie) {
    ...
    if (loopbackMint) cachedCoreAuthCookie = loopbackMint.pair
  }
  ...
  const handle = (req, res) => {
    ...
    const isCfTunnel = Boolean(tunnelPin && isCloudflareRequest(req.headers, req.socket && req.socket.remoteAddress, options.trustedProxyCidrs))
    ...
    function dispatch(attempt) {
      ...
      const upstream = http.request({
        ...
        if (answer.headers && answer.headers['set-cookie']) {
          ...
          if (picked) cachedCoreAuthCookie = picked
        }
        queued = false
