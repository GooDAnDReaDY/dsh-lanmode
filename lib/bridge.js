...
export function startDirectBridge(ctx, options) {
  ...
  const isPasswordAuth = () => options.passwordAuth === true
  const getAuthUser = () => options.authUser || 'admin'
  const version = options.version || '0.7.13'
  ...
  const handle = (req, res) => {
    ...
    // #103: Аутентификация по логину и паролю
    ...
    const upstream = http.request({
      ...
      if (!patching) {
        ...
        // #119: Пропуск сжатия для мелких ответов (< 1024 байт) при известном размере
  ...
  const upgrade = (req, socket, head) => {
    ...
    // #103: Аутентификация по логину и паролю для WebSocket
    if (isPasswordAuth() && authManager) {
      const token = authManager.extractToken(req)
      const session = token ? authManager.validateSession(token) : null
      if (!session) {
