  ...
  try {
    if (typeof window !== 'undefined' && window.fetch) {
      ...
      var promptPinModal = function (onEntered) {
        ...
        backdrop.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;background:var(--dsw-alias-mask-bg, rgba(0,0,0,0.75));z-index:20000;display:flex;align-items:center;justify-content:center;padding:16px;'
        var card = document.createElement('div')
        card.style.cssText = 'background:var(--dsw-alias-bg-layer-2, var(--dsw-alias-bg-layer-3, #1e1e2e));border:1px solid var(--dsw-alias-border-l2, var(--dsw-alias-border-l1, #313244));border-radius:16px;padding:24px;max-width:320px;width:100%;color:var(--dsw-alias-label-primary, #cdd6f4);box-shadow:var(--dsw-alias-shadow-l3, 0 8px 32px rgba(0,0,0,0.6));text-align:center;'
        card.innerHTML = '<div style="font-weight:600;font-size:16px;margin-bottom:8px;color:var(--dsw-alias-label-primary, #cdd6f4);">🔒 LAN PIN Required</div>'
          + '<div style="font-size:12px;color:var(--dsw-alias-label-secondary, #a6adc8);margin-bottom:16px;">Privileged settings are protected by the host PIN.</div>'
          + '<input id="dsh-pin-input" type="password" maxlength="12" placeholder="PIN" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--dsw-alias-border-l2, #45475a);background:var(--dsw-alias-bg-layer-3, var(--dsw-alias-bg-layer-1, #181825));color:var(--dsw-alias-label-primary, #fff);font-size:18px;text-align:center;letter-spacing:4px;box-sizing:border-box;margin-bottom:16px;outline:none;" />'
          + '<div style="display:flex;gap:8px;">'
          + '<button id="dsh-pin-cancel" type="button" style="flex:1;padding:8px;border-radius:8px;background:var(--dsw-alias-fill-subtle, var(--dsw-alias-bg-layer-3, #313244));color:var(--dsw-alias-label-primary, #cdd6f4);border:1px solid var(--dsw-alias-border-l2, transparent);cursor:pointer;">Cancel</button>'
          + '<button id="dsh-pin-submit" type="button" style="flex:1;padding:8px;border-radius:8px;background:var(--dsw-alias-state-brand-primary, #6366f1);color:var(--dsw-alias-label-on-brand, #fff);border:none;cursor:pointer;font-weight:600;">Unlock</button>'
          + '</div>'
      ...
      window.fetch = function (resource, init) {
        return origFetch.apply(this, arguments).then(function (response) {
          if (response.status === 401 && response.headers && response.headers.get('x-dsh-auth-required') === '1') {
            try { window.location.href = '/' } catch (err) { /* bestEffort */ void err }
            return response
          }
