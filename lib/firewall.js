...
export async function ensurePortAllowed(port) {
  ...
  if (platform === 'win32') {
    try {
      ...
      if (add.error) {
        return { managed: false, open: false, platform: 'win32', detail: 'Требуются права администратора для добавления правила брандмауэра' }
      }
    ...
    try {
      ...
      if (!ufwCheck.error && ufwCheck.stdout.trim()) {
        ...
        if (!ufwStatus.error && ufwStatus.stdout.includes('active')) {
          ...
          if (res.error) {
            return { managed: false, open: false, platform: 'linux-ufw', detail: 'Требуются права администратора (sudo) для ufw allow' }
          }
      ...
      if (!fwdCheck.error && fwdCheck.stdout.trim()) {
        ...
        if (!fwdState.error && fwdState.stdout.includes('running')) {
          ...
          if (res.error) {
            return { managed: false, open: false, platform: 'linux-firewalld', detail: 'Требуются права администратора для firewall-cmd' }
          }
