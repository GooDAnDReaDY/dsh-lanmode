...
import { generateQRSvg, generateAsciiQR } from './qr.js'
...
export function apply(ctx, config) {
  ...
  const settingsService = (ctx.settings && typeof ctx.settings.register === 'function')
    ? ctx.settings
    : null
  ...
  if (settingsService) {
    try {
      ...
      if (val && typeof val === 'object' && Object.keys(val).length > 0) {
        ...
        say('настройки загружены из службы (режим: ' + (effective.mode || 'auto') + ')')
      } else {
        configSource = (config && Object.keys(config).length > 0) ? 'row' : 'defaults'
        say('настройки получены из ' + configSource + ' (режим: ' + (effective.mode || 'auto') + ')')
      }
      ...
      configWarning = 'Ошибка регистрации настроек: ' + String(err.message || err)
      say('ВНИМАНИЕ: ' + configWarning + ' — плагин работает на умолчаниях')
    }
  } else {
    configSource = (config && Object.keys(config).length > 0) ? 'row' : 'defaults'
    say('служба настроек недоступна, запуск из ' + configSource + ' (режим: ' + (effective.mode || 'auto') + ')')
  }
...
async function raiseListener(ctx, config, state) {
    ...
    if (!dir) {
      ...
      try {
        ...
        say((made.issued ? 'выпущен' : 'взят') + ' сертификат на ' + certHosts.length
          + ' имён (включая ' + mdnsHost + '), отпечаток ' + made.fingerprint)
        if (made.caCert) {
  ...
  try {
    ...
    say('QR-код для быстрого входа со смартфона:' + generateAsciiQR(primaryUrl))
  } catch (_) {}

  // #59, #60: Автоматическое открытие порта в брандмауэре хоста
  try {
    ensurePortAllowed(port).then((fw) => {
      if (fw.managed && fw.detail) say('Брандмауэр: ' + fw.detail)
    }).catch(() => {})
  } catch (_) {}
...
function start(ctx, config, meta = {}) {
  ...
  }, 'dsh-lanmode: сброс реестра устройств на диск')
  ...
  if (!mdnsName.endsWith('.local')) {
    say(`[dsh-lanmode] Предупреждение: имя mDNS "${mdnsName}" обязано оканчиваться на .local. Добавлен суффикс: "${mdnsName}.local"`)
  ...
  if (config.mdns !== false) {
    ctx.effect(() => {
      ...
      if (addresses.length === 0) {
        say(`[dsh-lanmode] mDNS: все локальные адреса исключены списком allow (${config.allow.join(', ')}) — анонс ${mdnsName} не запущен`)
        state.mdns = false
        return
      }
  ...
  // Режим работы и управление слушателем
  let currentStop = () => {}
  ...
  const syncListener = async (cfg) => {
    ...
    if (mode === 'auto') {
      ...
      state.mode = mode
      state.modeReason = 'задан в настройках'
    }
  ...
  ctx.effect(() => {
    listenerAlive = true
    syncListener(config).catch((failed) => say('прямой режим не поднялся: ' + String(failed.message || failed)))
    return () => {
      listenerAlive = false
      if (currentStop) {
        currentStop()
  ...
  if (config.diagnostics !== false) {
    ...
    ctx.effect(() => ctx.webServer.register({
      ...
      handler: (req, res) => {
        const manifest = {
          ...
          shortcuts: [
            { name: 'Новый диалог', url: '/?new=1', description: 'Открыть новый диалог с агентом' },
          ],
        }
    ...
    // #63 Диагностический порт-проб (Self-Test)
    ctx.effect(() => ctx.webServer.register({
      kind: 'exact',
    ...
    }), 'dsh-lanmode: диагностический эндпоинт probe')

    // #51-#54 Реестр устройств и отзыв сессий
    ctx.effect(() => ctx.webServer.register({
      kind: 'exact',
    ...
    ctx.effect(() => ctx.webServer.register({
      ...
      handler: (req, res) => {
        ...
        res.writeHead(200, { 'content-type': 'application/json; charset=utf-8' })
        res.end(JSON.stringify({ ok: true, message: 'Все сессии отозваны' }))
      },
    }), 'dsh-lanmode: сброс всех сессий')

    // #103 Эндпоинты аутентификации login, logout, session
    ctx.effect(() => ctx.webServer.register({
      kind: 'exact',
    ...
    }), 'dsh-lanmode: аутентификация login')
    ...
    }), 'dsh-lanmode: аутентификация logout')
    ...
    }), 'dsh-lanmode: статус аутентификации session')

    // #46-#50 Управление WAN-туннелями Cloudflare
    ctx.effect(() => ctx.webServer.register({
      kind: 'exact',
    ...
    }), 'dsh-lanmode: статус WAN туннеля')
    ...
    }), 'dsh-lanmode: переключение WAN туннеля')
  }
  ...
  return {
    ...
    onConfigUpdated: (updatedCfg) => {
      ...
      if (typeof syncListener === 'function') {
        syncListener(config).catch((err) => say('ошибка применения новых настроек: ' + String(err.message || err)))
      }
    },
  }
}
