...
export function healthPage(state) {
  ...
  return '<!doctype html><html lang="ru"><head><meta charset="utf-8">'
    ...
    + '<dl><dt>источник конфигурации</dt><dd>' + escapeHtml(report.configSource)
    + (report.configWarning ? ' <span style="color:#f87171;font-weight:600">(' + escapeHtml(report.configWarning) + ')</span>' : '') + '</dd>'
    + '<dt>режим</dt><dd>' + escapeHtml(report.mode)
    ...
    + '<dt>вход по паролю</dt><dd>' + (report.passwordAuth ? ('ВКЛЮЧЕН (пользователь: ' + escapeHtml(report.authUser) + ')') : 'выключен') + '</dd>'
