...
//
// Pure JavaScript implementation of ISO/IEC 18004 (versions 1-10)
// without external dependencies. Generates vector SVG for browsers,
// terminal, and mobile clients.

// Galois Field GF(256) with polynomial x^8 + x^4 + x^3 + x^2 + 1 (0x11d = 285)
...
// Version table (versions 1..10, error correction level M):
...
// Alignment pattern center positions for versions 1..10
...
function encodeData(text, version) {
  ...
  buffer.put(0x4, 4)
  // Data length (8 bits for versions 1-9, 16 bits for version 10+)
  ...
  // Divide into blocks and compute Reed-Solomon error correction
...
function findBestVersion(text) {
  ...
  throw new Error('Data is too large for QR generator (maximum ~213 bytes)')
}
...
class QRCodeMatrix {
  ...
  setupPositionPatterns() {
    ...
    for (let i = 0; i < positions.length; i++) {
      for (let j = 0; j < positions.length; j++) {
        const r = positions[i]
        const c = positions[j]
        // Skip if intersecting finder patterns
    ...
    // Reserve format areas around finder patterns
    for (let i = 0; i < 9; i++) {
  ...
  writeFormatInformation(maskPattern) {
    // Error correction level M = 00, Mask = maskPattern
...
/**
 * Generate boolean matrix for QR code string.
 * @param {string} text String to encode
 * @returns {boolean[][]} 2D boolean array (true = black dot)
 */
...
/**
 * Generate clean SVG string for QR code.
 * @param {string} text Text/URL to encode
 * @param {object} [options] Options (margin, size, dark, light)
...
/**
 * Generate compact ASCII QR code for terminal (#35).
 * @param {string} text Text/URL to encode
 * @returns {string} String with block characters
 */
...
// In-memory LRU cache for generated SVG QR codes (#117)
const QR_CACHE_MAX = 50
const qrSvgCache = new Map()

/**
 * Generate clean SVG QR code with transparent LRU caching (#117).
 * @param {string} text Text/URL to encode
 * @param {object} [options] Options (margin, size, dark, light)
 * @returns {string} Valid SVG string
 */
export function generateCachedQRSvg(text, options = {}) {
  const margin = options.margin ?? 4
  const size = options.size ?? 256
  const dark = options.dark ?? '#000000'
  const light = options.light ?? '#ffffff'
  const key = String(text) + '|' + margin + '|' + size + '|' + dark + '|' + light

  if (qrSvgCache.has(key)) {
    const cached = qrSvgCache.get(key)
    // LRU touch
    qrSvgCache.delete(key)
    qrSvgCache.set(key, cached)
    return cached
  }

  const svg = generateQRSvg(text, options)
  qrSvgCache.set(key, svg)
  if (qrSvgCache.size > QR_CACHE_MAX) {
    const oldestKey = qrSvgCache.keys().next().value
    qrSvgCache.delete(oldestKey)
  }
  return svg
}

/**
 * Drop drawn SVG entries. Tests call this between cases.
 * The encoded link stays the same, so the next draw of that link matches
 * the deleted picture. A different link is stored under its own key.
 */
