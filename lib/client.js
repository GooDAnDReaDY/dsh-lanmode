...
.lm-btn-danger { background:rgba(239,68,68,0.12); color:#ef4444; border-color:rgba(239,68,68,0.3) }
.lm-btn-danger:hover { background:rgba(239,68,68,0.22) }
...
.lm-form-box { background:var(--dsw-alias-bg-layer-2); border-radius:10px; border:1px solid var(--dsw-alias-border-l2); padding:14px; display:flex; flex-direction:column; gap:12px; margin-top:8px }
.lm-form-title { font-size:14px; font-weight:600; color:var(--dsw-alias-label-primary) }
.lm-form-desc { font-size:12px; color:var(--dsw-alias-label-secondary); line-height:1.4 }
...
.lm-input { appearance:none; width:100%; box-sizing:border-box; background:var(--dsw-alias-bg-layer-3); border:1px solid var(--dsw-alias-border-l2); border-radius:8px; padding:6px 10px; color:var(--dsw-alias-label-primary); font:inherit; font-size:13px }
.lm-input:focus { outline:none; border-color:var(--dsw-alias-label-primary) }
.lm-select { appearance:none; width:100%; box-sizing:border-box; background:var(--dsw-alias-bg-layer-3); border:1px solid var(--dsw-alias-border-l2); border-radius:8px; padding:6px 10px; color:var(--dsw-alias-label-primary); font:inherit; font-size:13px; cursor:pointer }
.lm-select:focus { outline:none; border-color:var(--dsw-alias-label-primary) }
...
`
    ...
    function LanModeCard(props) {
      ...
      // Issue #95: Привязка полей Config к settingsScope
      var scope = useMemo(function () {
        return (ctx && ctx.settingsScope && typeof ctx.settingsScope.bind === "function")
                      ...
                      React.createElement(
                        ...
                        React.createElement(
                          ...
                          React.createElement(
                            ...
                            React.createElement('option', { value: 'auto' }, 'auto (автоматический)'),
                            React.createElement('option', { value: 'direct' }, 'direct (прямой слушатель)'),
                            React.createElement('option', { value: 'proxy' }, 'proxy (обратный прокси)')
                          )
                        ),
                        React.createElement(
                          'div',
                          { className: 'lm-field' },
                          React.createElement('label', { className: 'lm-field-label' }, 'Сетевой порт (directPort)'),
                        ...
                        React.createElement(
                          'div',
                          { className: 'lm-field' },
                          React.createElement('label', { className: 'lm-field-label' }, 'Шифрование TLS (tls)'),
                          React.createElement(
                            'select',
                            {
                              className: 'lm-select',
                            ...
                            React.createElement('option', { value: 'self-signed' }, 'self-signed (Root CA + серверный)'),
                            React.createElement('option', { value: 'files' }, 'files (из файлов ключей)'),
                            React.createElement('option', { value: 'off' }, 'off (отключено / голый HTTP)')
                          )
                        ),
                        React.createElement(
                          'div',
                          ...
                          React.createElement('input', {
                            ...
                            placeholder: 'Имя секрета в credentials / ENV',
                            onChange: function (e) {
                              var val = e.target.value
                        ...
                        React.createElement(
                          ...
                          React.createElement('label', { className: 'lm-field-label' }, 'Разрешенные адреса/подсети (allow)'),
                          React.createElement('input', {
                            type: 'text',
                            className: 'lm-input',
                            value: draft.allow,
                            placeholder: '192.168.1.0/24, 127.0.0.1 (пусто = доступ для всех)',
                            onChange: function (e) {
                              var val = e.target.value
                        ...
                        React.createElement(
                          'div',
                          { className: 'lm-field lm-field-full', style: { borderTop: '1px solid var(--dsw-alias-border-l1, #313244)', paddingTop: '10px', marginTop: '6px' } },
                          React.createElement(
                            'label',
                            ...
                            React.createElement('span', null, '🛡 Включить вход по логину и паролю (защита любого входа в DSH)')
                          ),
                          React.createElement(
                            'div',
                            { className: 'lm-hint', style: { marginLeft: '24px', marginTop: '2px' } },
                            'Блокирует доступ к веб-интерфейсу и API до авторизации через фирменную страницу входа. Сессия действует 30 дней.'
                          )
                        ),
                        Boolean(draft.passwordAuth) && React.createElement(
                          ...
                          React.createElement(
                            'div',
                            { className: 'lm-field lm-field-full' },
                            React.createElement('label', { className: 'lm-field-label' }, 'Имя секрета в credentials (authPasswordRef, опционально)'),
                            React.createElement('input', {
                              type: 'text',
                              className: 'lm-input',
                              value: draft.authPasswordRef,
                              placeholder: 'DSH_AUTH_PASSWORD или имя секрета в защищённом хранилище',
                              onChange: function (e) {
                                var val = e.target.value
                            ...
                            React.createElement(
                              'div',
                              { className: 'lm-hint' },
                              'Если задано, пароль будет читаться из защищённого хранилища credentials или переменных окружения.'
                            )
                          )
                        )
                      ),
