...
`
    ...
    function QuickQrPopover(props) {
      ...
      // #117: Замер RTT только когда модалка открыта или вкладка стала видимой
      useEffect(function () {
        if (!open) return
            ...
            React.createElement(
              ...
              (typeof navigator !== 'undefined' && typeof navigator.share === 'function') && React.createElement(
                'button',
                {
                  ...
                  title: 'Поделиться адресом через систему',
                  onClick: function () {
                    try {
                      navigator.share({ title: 'DeepSeek Harness', url: window.location.origin }).catch(function () {})
                    } catch (_) {}
                  },
                },
                'Поделиться ↗',
