...
`
    ...
    module.exports.apply = function apply(ctx) {
      ...
      // Регистрация карточки плагина в настройках и кнопки QR в подвале боковой панели
      if (ctx.slots && ctx.slots.register) {
        try {
          ...
          console.warn('[dsh-lanmode] Не удалось зарегистрировать слот settings.plugin.item:', err)
        }
        try {
          ctx.slots.register(
            {
          ...
          // eslint-disable-next-line no-console
          console.warn('[dsh-lanmode] Не удалось зарегистрировать слот sidebar.footer.action:', err)
        }
      }
