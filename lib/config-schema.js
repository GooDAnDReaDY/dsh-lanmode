...
export const Config = z.object({
  ...
  tlsSites: z
    ...
    .description('mode=direct: extra certificates selected by TLS server name. Unknown names and IP addresses keep the default certificate.')
    .default([]),
  allow: z
    .array(z.string())
  ...
  trustedProxyCidrs: z
    .array(z.string())
    .description('CIDR ranges of reverse proxies that may set X-Forwarded-For / CF-Connecting-IP. Direct peers outside this list always use socket remoteAddress.')
  ...
  disabledUsers: z
    .array(z.string())
    .description('Usernames whose sessions are revoked on the next sweep.')
    .default([]),
  authSessionDays: z
    .number()
  ...
  publicHost: z
    .string()
    .description('Public hostname shown on the login card so users can confirm they are not on a phishing proxy. Empty uses the request Host.')
    .default(''),
  streamTimeoutMs: z
    .number()
    .description('mode=direct: how long a single request may take. 0 means no limit.')
    .default(0),
  adaptiveCompression: z
    .boolean()
    .description('mode=direct: gzip/brotli only for non-local clients. Local loopback/LAN skips compression to save host CPU (remoteOnly).')
    .default(true),
  diagnostics: z
    .boolean()
