...
export function isTrustedUpdateRequest(request, { state, config, clientIp, role } = {}) {
  ...
  // For non-loopback clients, verify administrative access
  if (state || config) {
    const auth = verifyAdminAccess(request, { state, config, clientIp: remoteIp, role })
    return auth.allowed
  }
    ...
    handler: async (request, response) => {
      try {
        ...
        const authContext = {
          ...
          clientIp,
          role: options.resolveClientRole ? options.resolveClientRole(clientIp, options.config) : 'admin',
        }
