...
// 1. Local Root Certificate Authority (100-year lifetime) - lanmode-ca.pem
...
/** Server and CA lifetime in days. A saved pair is kept until it nears expiry. */
const LIFETIME_DAYS = 36500

/** Root CA certificate lifetime in days (100 years). */
const CA_LIFETIME_DAYS = 36500
...
/** DNS names that resolve to an IP via sslip.io and nip.io. */
export function dnsAliases(address) {
  const clean = String(address ?? '').trim()
  const ipv4 = /^[0-9.]+$/.test(clean)
  const ipv6 = clean.includes(':')
  if (!ipv4 && !ipv6) return []
  const label = ipv6 ? clean.replaceAll(':', '-') : clean
  return [label + '.sslip.io', label + '.nip.io']
}
...
/** Host names for a certificate SAN: local addresses plus sslip.io and nip.io aliases. */
export function certificateHosts(extra = []) {
  const found = new Set(localAddresses(extra))
  for (const name of [...found]) {
    for (const alias of dnsAliases(name)) found.add(alias)
  }
  return [...found]
}
...
async function ensureRootCA(dir, log) {
  ...
  if (fs.existsSync(caKeyPath) && fs.existsSync(caCertPath)) {
    try {
      ...
      const validUntil = typeof info.expires === 'number'
        ? info.expires
        : (info.validTo instanceof Date ? info.validTo.getTime() : 0)
      if (validUntil > Date.now() + 30 * 86400 * 1000) {
  ...
  log('generating dsh-lanmode Local Root CA (100-year validity)...')
  await run('openssl', [
...
export async function ensureCertificate(options) {
  ...
  if (fs.existsSync(certPath) && fs.existsSync(keyPath)) {
    try {
      ...
      if (info.validTo.getTime() - now >= RENEW_BEFORE_MS) {
        safeChmod(keyPath, 0o600, options.log)
        safeChmod(certPath, 0o600, options.log)
        return {
          cert,
      ...
      options.log('server certificate renewing: nearing expiration')
    } catch (unreadable) {
...
export function readRootCA(dir) {
  if (!dir) return null
  for (const name of ['lanmode-ca.crt', 'lanmode-ca.pem']) {
    const caCertPath = path.join(dir, name)
    if (!fs.existsSync(caCertPath)) continue
    try {
