...
/**
 * Интерфейсы, живущие ровно столько, сколько живёт контейнер.
 *
 * Докер заводит виртуальную пару на каждый контейнер и мост на каждую сеть.
 * Адреса на них видны только изнутри контейнера, а появляются и исчезают по
 * десятку за день.
 */
const EPHEMERAL_INTERFACE = /^(veth|br-|docker|virbr)/i

/**
 * Связь-локальный адрес: браузеру недоступен в принципе.
 *
 * Такому адресу нужен индекс зоны — `[fe80::1%eth0]`, — и ни один человек
 * этого в строку не набирает.
 */
 ...
 *
 * В сертификат идут все сразу: браузер сверяет тот адрес, который набрали в
 * строке, и сертификат, выписанный на одно имя, ругается на все остальные —
 * даже после того, как его один раз приняли.
 *
 * Но «все» — это не каждый адрес из системы. Летучие сюда попадать не должны,
 * и дело не в размере списка: сертификат считается негодным, как только на
 * машине появился адрес, которого в нём нет. Один поднятый контейнер — и
 * сертификат перевыпускается, отпечаток меняется, браузер требует подтвердить
 * страницу заново. Обещание «подтвердить один раз» держится ровно до
 * следующего `docker compose up`.
 *
 * Кому нужен адрес, которого здесь нет, — для этого есть `tlsHosts`.
 */
