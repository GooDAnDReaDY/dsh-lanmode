import { isLocalLanClient } from './network-interfaces.js'
 ...
 */
/**
 * Checks whether a given hostname or authority belongs to trusted local or LAN set.
 * Protects against DNS rebinding attacks (Issue #307).
 *
 * @param {string} hostOrHostname
 * @param {object} [options]
 * @returns {boolean}
 */
export function isTrustedLocalAuthority(hostOrHostname, options = {}) {
  if (!hostOrHostname || typeof hostOrHostname !== 'string') return false
  const clean = hostOrHostname.trim().toLowerCase()
  let hostname = clean
  if (clean.startsWith('[')) {
    const endBracket = clean.indexOf(']')
    if (endBracket !== -1) {
      hostname = clean.slice(1, endBracket)
    }
  } else {
    const firstColon = clean.indexOf(':')
    const lastColon = clean.lastIndexOf(':')
    if (firstColon !== -1 && firstColon === lastColon) {
      hostname = clean.slice(0, firstColon)
    }
  }

  if (hostname === 'localhost' || hostname === 'dsh.local') return true
  if (isLoopbackAddress(hostname)) return true
  if (isLocalLanClient(hostname)) return true

  // sslip.io / nip.io wildcards for local LAN IPs
  const dotPattern = /^(\d{1,3}(?:\.\d{1,3}){3})\.(?:sslip\.io|nip\.io)$/
  const dashPattern = /^(\d{1,3}(?:-\d{1,3}){3})\.(?:sslip\.io|nip\.io)$/
  const dotMatch = hostname.match(dotPattern)
  if (dotMatch && isLocalLanClient(dotMatch[1])) return true
  const dashMatch = hostname.match(dashPattern)
  if (dashMatch) {
    const ip = dashMatch[1].replace(/-/g, '.')
    if (isLocalLanClient(ip)) return true
  }

  // Explicit allowed hosts from options / config
  const custom = options.allowedHosts
    || options.config?.allowedHosts
    || options.config?.tlsHosts
    || options.state?.config?.allowedHosts
    || options.state?.config?.tlsHosts
    || []
  if (Array.isArray(custom)) {
    for (const item of custom) {
      if (typeof item === 'string' && item.trim().toLowerCase() === hostname) return true
    }
  }
...
/**
 * Validates request Origin and Sec-Fetch-Site headers to guard against CSRF and DNS-rebinding.
 * @param {object} req
 * @param {object} [options]
 * @returns {boolean}
 */
export function isTrustedSameOrigin(req, options = {}) {
  ...
  if (origin && host) {
    try {
      ...
      // Both Origin and Host must belong to trusted local/LAN authorities (Issue #307)
      if (!isTrustedLocalAuthority(originUrl.hostname, options) || !isTrustedLocalAuthority(hostName, options)) {
        return false
      }
    ...
    try {
      const originUrl = new URL(origin)
      if (!isTrustedLocalAuthority(originUrl.hostname, options)) return false
    } catch (_) {
      return false
    }
  } else if (host) {
    if (!isTrustedLocalAuthority(host, options)) {
      return false
    }
  }
  return true
...
export function verifyAdminAccess(req, { state, config, clientIp, role }) {
  ...
  // 2. Anti-CSRF and DNS-rebinding protection for state-mutating requests
  const method = req?.method || 'GET'
  if (method !== 'GET' && method !== 'HEAD' && method !== 'OPTIONS') {
    if (!isTrustedSameOrigin(req, { state, config })) {
      return {
        ok: false,
        status: 403,
  ...
  // Allow loopback calls ONLY if Host header is trusted (prevents DNS-rebinding attacking loopback #307)
  if (isLoopbackAddress(clientIp)) {
    if (req?.headers?.host && !isTrustedLocalAuthority(req.headers.host, { state, config })) {
      return {
        ok: false,
        status: 403,
        error: 'Forbidden: Untrusted host authority for administrative access',
      }
    }
