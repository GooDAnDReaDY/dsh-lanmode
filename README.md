...
Доступ по локальной сети и поддержка браузерных медиа-API для веб-интерфейса DeepSeek Harness.

Обеспечивает корректную работу диктовки, микрофона и медиа-функций при доступе с телефонов и планшетов в подсетях `192.168.x.x` / `10.x.x.x` без HTTPS.
...
DeepSeek Harness 局域网多端访问优化与 HTTP 媒体接口兼容插件。

解除移动端及平板通过 `192.168.x.x` / `10.x.x.x` 等内网 IP 在纯 HTTP 环境下访问时浏览器对录音及语音 API 的安全阻断。
